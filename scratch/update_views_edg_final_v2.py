...
content = replace_block(content, "def crear_recepcion(request):", "return render(request, 'inventario/crear_recepcion.html', contexto)", crear_recepcion_new)
...
content = replace_block(content, "def desglosar_entrada(request, detalle_id):", "return redirect('reportes_pendientes')", desglosar_entrada_new)
...
import re
old_consumo = re.compile(r"despachos_list = SalidaMaterialDetalle\.objects\.exclude\(.*?Q\(salida__nro_rim__startswith='AJUSTE-MIG'\) \| Q\(salida__departamento='MIGRACIN'\).*?\)\.select_related\(", re.DOTALL)
...
content = old_consumo.sub(new_consumo, content)

old_export = re.compile(r"despachos = SalidaMaterialDetalle\.objects\.exclude\(.*?Q\(salida__nro_rim__startswith='AJUSTE-MIG'\) \| Q\(salida__departamento='MIGRACIN'\).*?\)\.select_related\(", re.DOTALL)
content = old_export.sub(new_consumo.replace("despachos_list", "despachos"), content)
